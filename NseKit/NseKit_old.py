...
class Nse:
    ...
    def index_live_contribution(self, *args, Index: str = "NIFTY 50", Mode: str = "First Five"):
        ...
        try:
            ...
            print(f"Error fetching index contribution data: {e}")
            return None
    ...
    def index_historical_data(self, index: str, *args, from_date=None, to_date=None, period=None):
        ...
        """
        Fetch historical NSE index price data.
        Automatically handles argument detection, periods,
        and safe chunk downloads.

        Example
        -------
        index_historical_data("NIFTY 50","1Y")
        index_historical_data("NIFTY 50","01-01-2024","01-01-2025")
        """
        ...
        base_api = "https://www.nseindia.com/api/historicalOR/indicesHistory?indexType={}&from={}&to={}"
        ...
        while start_dt <= end_dt:
            ...
            if not success:
                ...
                fail_chunks.append(
                    f"{chunk_start.strftime('%d-%b-%Y')} → {chunk_end.strftime('%d-%b-%Y')}"
                )
    ...
    def cm_live_hist_br_sr(self, *args, from_date=None, to_date=None, symbol=None):
        ...
        try:
            ...
            if isinstance(data, list) and len(data) > 0:
                ...
                scope = symbol or "All symbols"
                print(f"ℹ️  No BSR Reports found for {scope} between {from_date or '-'} and {to_date or '-'}")
                return None

        except (requests.RequestException, ValueError, KeyError) as e:
            print(f"❌ Error fetching BSR Reports: {e}")
            return None
    ...
    # def html_tables(self, url: str, show_tables: bool = False):
    #     """
    #     Fetch tables from NSE iXBRL HTML filing.

    #     Parameters
    #     ----------
    #     url : str
    #         iXBRL HTML file URL from NSE archives
    #     show_tables : bool
    #         If True, prints preview of each table
    ...
    #     self.rotate_user_agent()  # Rotate UA like other NSE functions

    #     try:

    #         # Reference call (NSE requires cookies)
    #         ref_url = "https://www.nseindia.com/"
    #         ref_response = self.session.get(ref_url, headers = self.headers , timeout=20)
    #         ref_response.raise_for_status()

    #         # Fetch iXBRL page
    #         response = self.session.get(url, headers = self.headers, cookies=ref_response.cookies.get_dict(), timeout=30)
    #         response.raise_for_status()
    ...
    #     except (requests.RequestException, ValueError) as e:
    #         print(f"Error fetching HTML tables: {e}")
    #         return None

    def html_tables(self, url: str, show_tables: bool = False, output: str = "json"):
        """
        Fetch tables from NSE iXBRL HTML filing.
        ...
        show_tables : bool
            If True, prints preview of each table
        output : str
            "dataframe" (default) or "json"

        Returns
        -------
        list[pd.DataFrame] | list[dict] | None
        """
        from io import StringIO
        ...
        try:
            # Step 1: Reference call (required for NSE cookies)
            ref_url = 'https://www.nseindia.com/option-chain'
            ref_response = self.session.get( ref_url, headers=self.headers, timeout=20)
            ref_response.raise_for_status()

            # Step 2: Fetch iXBRL page
            response = self.session.get(url, headers=self.headers, cookies=ref_response.cookies.get_dict(), timeout=30)
            response.raise_for_status()
